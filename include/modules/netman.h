/*
	Modified Simple Network Library from http://www.gaffer.org/networking-for-game-programmers
	Original Author: Glenn Fiedler <gaffer@gaffer.org>
...
// Detect current platform (windows, mac or unix)
#if defined(_WIN32)
...
#elif PLATFORM == PLATFORM_OSX || PLATFORM == PLATFORM_LINUX
...
// Check for packet sequence rolling over and restarting the sequence
...
public:
    ...
    void Close();
    bool IsOpen() const;
    bool Send(const Address & destination, const void*, int);
    int Receive(Address & sender, void*, int);
...
public:
    ...
    static void process_ack(unsigned int ack, unsigned int ack_bits, PacketQueue & pending_ack_queue, PacketQueue & acked_queue,
...
// Create "virtual" connections for UDP sockets
class Connection {
public:
    enum Mode {
        None,
    ...
    Connection(unsigned int protoID, float timeout, unsigned int max_sequence = 0xFFFFFFFF) {
    ...
    // Start virtual connection on specified port
    bool Start(int port) {
        assert(!running);
        printf("Started connection on port %d\n", port);
        if (!socket.Open(port))
            return false;
        running = true;
        return true;
    }
    // Stop virtual connection
    void Stop() {
        assert(running);
    ...
    // Start virtual server connection
    void Listen() {
    ...
    // Start virtual client connection
    ...
    bool SendPacket(const unsigned char*, int);
    // Receive modified UDP packets with headers
    int ReceivePacket(unsigned char*, int);
    // Update connection time length and check for timeouts
    void Update(float);

    int GetHeaderSize() const {	return 4 + reliabilitySystem.GetHeaderSize(); }
...
private:
    ...
    float timeout, timeoutAccumulator;
    bool running;
...
public:
    ...
    // Sort packet queue with most recent packet first after rolling over
